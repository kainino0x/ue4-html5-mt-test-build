...
var Module = {
	...
	UE4_indexedDBName: 'UE4_assetDatabase_html5_mt_test', // this should be an ascii ID string without special characters that is unique to the project that is being packaged
	UE4_indexedDBVersion: 201810250205, // Bump this number to invalidate existing IDB storages in browsers.
};
...
Module.arguments = ['../../../html5_mt_test/html5_mt_test.uproject','-stdout',];
...
const targetOffscreenCanvas = Module['UE4_MultiThreaded'];
...
// Non-growable memory.
var MIN_MEMORY = 512 * 1024 * 1024; // 512MB
var MAX_MEMORY = 512 * 1024 * 1024; // 512MB
...
//		Module['preinitializedWebGLContext'] = null; // TODO: Currently can't preinitialize a context when OffscreenCanvas is used.
	...
	function withIndexedDB(db) {
		...
		// ----------------------------------------
		// MORE JS
		var dataJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('html5_mt_test.data.js'));
...
/* // The following code would download and store the .data file as a Blob, which should be more efficient than loading an ArrayBuffer. However that seems to be buggy, so avoid it for now.
			fetchOrDownloadAndStore(db, Module.locateFile('html5_mt_test.data')).then(function(dataBlob) {
				return readBlobToArrayBuffer(dataBlob).then(function(dataArrayBuffer) {
					Module['preloadedPackages'] = {};
					Module['preloadedPackages'][Module.locateFile('html5_mt_test.data')] = dataArrayBuffer;
...
// Instead as a fallback, download as ArrayBuffer. (TODO: Figure out the bugs with the above, and switch to using that one instead)
			fetchOrDownloadAndStore(db, Module.locateFile('html5_mt_test.data'), 'arraybuffer').then(function(dataArrayBuffer) {
				Module['preloadedPackages'] = {};
				Module['preloadedPackages'][Module.locateFile('html5_mt_test.data')] = dataArrayBuffer;
				return dataJsDownload.then(addScriptToDom);
			});
